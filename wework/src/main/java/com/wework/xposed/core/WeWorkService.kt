...
class WeWorkService(private val context: Context) : IWeWorkService.Stub() {
    override fun setWorkhHandler(handl: IWeWorkHandler?) {
        this.handler = handl
    }

    var handler: IWeWorkHandler? = null

    override fun getWorkHandler(): IWeWorkHandler? {
        return this.handler
    }
    ...
    private fun call(listener: Listener, senderType: Int, event: String, data: String) {
        try {
            ...
            listener.callback.callback(senderType, event, data)
        } catch (ex: Exception) {
            ex.printStackTrace()
